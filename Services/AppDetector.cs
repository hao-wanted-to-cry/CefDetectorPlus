    ...
    public class AppDetector
    {
            ...
            string? icon, string? exe, string engine)
        {
            ...
            _ = Task.Run(async () =>
            {
                ...
                try { app.SizeBytes = CalcSize(location); }
                catch { }
                finally { _sizeSemaphore.Release(); }
            });
        }
        ...
        /// 仅搜索根目录 + 一级子目录，避免 AllDirectories 对大目录（Steam 库等）造成巨大内存消耗。
        /// </summary>
        private static string? FindExe(string path)
        {
            try
            {
                var allExes = new List<(string Path, string Name)>();

                // 根目录
                AddNonSystemExes(path, allExes);
                ...
                if (allExes.Count == 0)
                {
                    ...
                    {
                        try { AddNonSystemExes(subDir, allExes); }
                        catch { }
                        if (allExes.Count > 0) break;
                    }
                }
        ...
        private static void AddNonSystemExes(string dir, List<(string Path, string Name)> results)
        {
            try
            {
